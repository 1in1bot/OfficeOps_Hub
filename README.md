...
| v1.11.0 | 프로젝트 버전 관리 기준 반영 | 프로젝트 전체 버전 시작점과 major/minor 버전 상승 기준을 추가하고 관련 문서 링크 갱신 | 없음 |
| v1.12.0 | 화면 흐름 프로토타입 조작 개선 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 버전으로 갱신 | 없음 |
| v1.12.1 | 화면 흐름 프로토타입 사용성 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.0 | 화면 흐름 프로토타입 상호작용 개선 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 minor 버전으로 갱신 | 없음 |
| v1.13.1 | 화면 흐름 프로토타입 카드 이동 방식 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.2 | 화면 흐름 프로토타입 자동 정렬 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.3 | 화면 흐름 프로토타입 자유 배치와 충돌 정렬 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.4 | 화면 흐름 프로토타입 기본 배치와 밀림 방식 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.5 | 화면 흐름 프로토타입 자석형 슬롯 배치 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.6 | 화면 흐름 프로토타입 기본 배치와 끼워넣기 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.7 | 화면 흐름 프로토타입 공통 카드 배치 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.8 | 화면 흐름 프로토타입 기본 배치 재정렬 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.13.9 | 화면 흐름 프로토타입 카테고리 영역 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.14.0 | 화면 흐름 프로토타입 카테고리별 2줄 배치 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 minor 버전으로 갱신 | 없음 |
| v1.14.1 | 화면 흐름 프로토타입 카테고리별 세로 2열 배치 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.14.2 | 화면 흐름 프로토타입 카드 선택 동작 보완 반영 | 프로젝트 개요서와 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
...
현재 프로젝트 전체 버전은 `0.1.0`입니다.

- 프로젝트 전체 버전은 `0.1.0`으로 시작합니다.
- 실제 기능 구현이 완료되어 첫 안정 버전으로 볼 수 있을 때 `1.0.0`으로 올립니다.
- minor 변경점은 `0.1.0` 단위로 올립니다.
- major 변경점은 `1.0.0` 단위로 올립니다.
- 문서별 산출물 버전은 프로젝트 전체 버전과 별도로 관리합니다.
...
| --- | --- |
| [Project Charter](./docs/project_charter/OfficeOps_hub_프로젝트개요서_v1.12.2.md) | 목표, 범위, 역할, 일정, 산출물을 포함한 공식 프로젝트 개요서 |
| [Detailed Plan](./docs/project_charter/OfficeOps_hub_상세계획서_v1.3.0.md) | 상세 기능 범위와 설계 방향 |
| [Team Charter](./docs/team_charter/OfficeOps_hub_팀운영문서_v1.9.0.md) | 개발 환경, 팀 협업 방식, Git/PR 규칙 |
...
| [화면 흐름도](./docs/specifications/OfficeOps_hub_화면흐름도_v1.6.2.md) | 현재 구현 범위 기준 화면 흐름 |
