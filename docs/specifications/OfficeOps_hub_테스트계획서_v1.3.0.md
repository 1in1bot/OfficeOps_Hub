...
| 버전 | 기준 | 수정 사항 | 삭제 사항 |
| --- | --- | --- | --- |
| v1.0.0 | 관리자 전용 계정 생성 반영 이전 테스트 계획 | 회원가입/로그인 테스트와 기존 권한 테스트 기준선 | 없음 |
| v1.1.0 | 관리자 전용 계정 생성 반영 | 필수 테스트 범위를 로그인/관리자 사용자 계정 생성으로 변경, `ROLE_ADMIN` 계정 생성 성공과 `ROLE_OPERATOR` 계정 생성 403 케이스 추가 | 정상 회원가입/중복 이메일 회원가입 테스트 표현 제거 |
...
| ID | 케이스 | 기대 결과 |
| --- | --- | --- |
| AUTH-TC-001 | ROLE_ADMIN 사용자 계정 생성 | 사용자 생성, 지정 권한, ACTIVE |
| AUTH-TC-002 | 중복 이메일 사용자 계정 생성 | Validation 에러 |
...
| PERM-TC-006 | ROLE_OPERATOR가 사용자 계정 생성 | 403 |
