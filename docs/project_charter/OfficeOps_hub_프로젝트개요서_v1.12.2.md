...
| v1.9.0 | 프로젝트 버전 관리 기준 반영 | 프로젝트 전체 버전 시작점과 major/minor 버전 상승 기준을 프로젝트 기본 정보에 추가 | 없음 |
| v1.10.0 | 화면 흐름 프로토타입 조작 개선 반영 | 참고 문서의 화면 흐름도 링크를 최신 버전으로 갱신 | 없음 |
| v1.10.1 | 화면 흐름 프로토타입 사용성 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.0 | 화면 흐름 프로토타입 상호작용 개선 반영 | 참고 문서의 화면 흐름도 링크를 최신 minor 버전으로 갱신 | 없음 |
| v1.11.1 | 화면 흐름 프로토타입 카드 이동 방식 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.2 | 화면 흐름 프로토타입 자동 정렬 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.3 | 화면 흐름 프로토타입 자유 배치와 충돌 정렬 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.4 | 화면 흐름 프로토타입 기본 배치와 밀림 방식 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.5 | 화면 흐름 프로토타입 자석형 슬롯 배치 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.6 | 화면 흐름 프로토타입 기본 배치와 끼워넣기 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.7 | 화면 흐름 프로토타입 공통 카드 배치 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.8 | 화면 흐름 프로토타입 기본 배치 재정렬 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.11.9 | 화면 흐름 프로토타입 카테고리 영역 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.12.0 | 화면 흐름 프로토타입 카테고리별 2줄 배치 반영 | 참고 문서의 화면 흐름도 링크를 최신 minor 버전으로 갱신 | 없음 |
| v1.12.1 | 화면 흐름 프로토타입 카테고리별 세로 2열 배치 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
| v1.12.2 | 화면 흐름 프로토타입 카드 선택 동작 보완 반영 | 참고 문서의 화면 흐름도 링크를 최신 patch 버전으로 갱신 | 없음 |
...
- 실제 기능 구현이 완료되어 첫 안정 버전으로 볼 수 있을 때 `1.0.0`으로 올린다.
- minor 변경점은 `0.1.0` 단위로 올린다.
- major 변경점은 `1.0.0` 단위로 올린다.
- 문서별 산출물 버전은 프로젝트 전체 버전과 별도로 관리한다.
- 기능 추가, 기능 수정, 문서 작업 시 관련 문서의 버전 이력을 확인하고 필요하면 즉시 버전 업한다.
...
| `docs/team_charter/OfficeOps_hub_팀운영문서_v1.9.0.md` | 팀 운영 및 협업 규칙 |
| `docs/specifications/OfficeOps_hub_화면흐름도_v1.6.2.md` | 화면 흐름도 |
