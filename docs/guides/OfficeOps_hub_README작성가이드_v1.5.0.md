# OfficeOps Hub README 작성 가이드

## 문서 버전 이력

| 버전 | 기준 | 수정 사항 | 삭제 사항 |
| --- | --- | --- | --- |
| v1.0.0 | 버전 단위 문서 관리 방식 도입 이전 README 가이드 | 기존 README 작성 기준선 | 없음 |
| v1.1.0 | 버전 단위 문서 관리 방식 도입 | README에도 문서 버전 이력과 수정/삭제 사항을 기록하도록 가이드 추가 | 없음 |
...
| v1.5.0 | README 작성 가이드 명칭 반영 | 문서 제목과 파일명을 README 작성 가이드 기준으로 수정 | `읽어보기 작성 가이드` 명칭 제거 |
...
README를 포함한 프로젝트 문서는 버전 단위로 변경 이력을 관리한다.
...
| v1.1.0 | 변경 반영 | 추가되거나 수정된 항목 요약 | 제거된 항목 요약 |

작성 규칙:

- 문서 상단에 `문서 버전 이력` 표를 둔다.
- 최초 정리 버전은 `v1.0.0`으로 시작한다.
- 문서별 변경 시점이 다를 수 있으므로 문서마다 최신 버전은 달라도 된다.
- 문서가 실제로 수정될 때만 해당 문서의 버전을 상승시킨다.
- 수정된 문서는 파일명 버전을 문서 내부 최신 버전과 동일하게 유지한다.
- 파일명 버전이 바뀐 문서를 참조하는 README와 관련 문서 링크만 즉시 함께 갱신한다.
- 같은 변경이 여러 문서에 영향을 주더라도 실제로 수정된 문서만 버전을 상승시킨다.
- 기능, API, 화면, 권한, 정책이 바뀌면 수정 사항에 기록한다.
- 삭제된 화면, API, 기능, 정책은 삭제 사항에 반드시 기록한다.
- 버전 이력은 요약만 작성하고, 상세 내용은 본문 섹션에 반영한다.
